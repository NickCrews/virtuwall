...
CameraPosition = {
    "x": -3.3, # distance of kinect sensor from wall.
...
kinect = kn.FakeKinect('recordings/a.rec')
frames = kinect.get_frames()
last_color = frames['registered']
def update():
    ...
    print(frames['color'])
    # if np.all(np.equal(last_color,frames['registered'])):
    #     print('frames are the saem')
    # last_color = frames['registered']
    ...
    v_rate = 6.0 # Rate that vertex sizes will increase as zoom level increases (adjust this to any desired value).
    ...
    out = depthMatrixToPointCloudPos(frames['undistorted'])
